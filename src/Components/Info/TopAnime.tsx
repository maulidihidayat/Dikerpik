...
export default async function topAnime() {
  ...
  const anime = await response.json();
    return (
        <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-6 items-center'>
        {anime.data.map((anime: animes) => {
            return (
                <div key={anime.mal_id} className='shadow-lg rounded-3xl border p-6 rounded-lg'>
